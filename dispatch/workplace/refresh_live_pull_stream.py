...
from module.request_module import RefreshStreamRequest

logger = logging.getLogger()
logger.setLevel(logging.INFO)


class RefreshLivePullStreamTaskWorker(BaseWorker):
    ...
    def main_handler(self):
        ...
        update_stream_request = RefreshStreamRequest(data)
        update_stream_data = update_stream_request.get_value(is_common_check=True)

        task_ids = update_stream_data['TaskIds']
        try:
            # 多线程分发任务，确保快速拉起转推任务
            ...
            while True:
                ...
                stream_worker = threading.Thread(target=self.update_live_stream,
                                                 args=(update_stream_data, task_list,))
    ...
    def update_live_stream(self, update_stream_data, task_list):
        ...
        for task_id in task_list:
            ...
            try:
                ...
                if task_data['SourceType'] == RtmpToRtmp:
                    raise NotSupportActionError()

                self.sig_publish(task_id, update_stream_data)
                self.task_map[task_id] = {
    ...
    def sig_publish(self, task_id, update_stream_data):
        sig_queue = event_queue_identifier(task_id)
        self.redis_client.rpush(key=sig_queue, list_of_value=[RefreshSig])
