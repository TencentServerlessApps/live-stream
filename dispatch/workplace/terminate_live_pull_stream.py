...
from module.request_module import TerminateStreamRequest
...
class TerminateLivePullStreamTaskWorker(BaseWorker):
    ...
    def main_handler(self):
        ...
        terminate_stream_request = TerminateStreamRequest(data)
        terminate_stream_data = terminate_stream_request.get_value(is_common_check=True)

        task_ids = terminate_stream_data['TaskIds']
        try:
            # 多线程分发任务，确保快速拉起转推任务
            ...
            while True:
                ...
                stream_worker = threading.Thread(target=self.terminate_live_stream,
                                                 args=(terminate_stream_data, task_list,))
    ...
    def terminate_live_stream(self, terminate_stream_data, task_list):
    ...
    def sig_publish(self, task_id):
        ...
        self.redis_client.rpush(key=sig_queue, list_of_value=[TerminateSig])
